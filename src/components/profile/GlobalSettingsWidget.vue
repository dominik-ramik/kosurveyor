<template>
    ...
    >
      <v-card-title class="d-flex align-center">
        ...
        <span class="text-subtitle-1 font-weight-bold">{{ profile.profile_name || "(unnamed)" }}</span>
        <span class="text-grey ml-2" v-if="showNames">({{ profile.form_id_stem || "(not set)" }})</span>        
      </v-card-title>
      <v-card-text>
        <div v-if="profile" class="text-body-2 d-flex flex-row ga-1 ml-10">
          <div v-if="profile.profile_author" class="ml-2">
            <strong>Author:</strong> {{ profile.profile_author }}
          </div>
          <div v-if="profile.profile_description" class="ml-4">
