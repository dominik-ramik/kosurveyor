<template>
  <div class="ml-4">
    ...
    <div class="mb-1 d-flex align-center gap-1">
      <span class="text-subtitle-2 text-grey-darken-1 font-weight-bold mr-2"
        >Appearance</span
      >
    ...
    >
      <v-btn value="default" size="small" class="flex-grow-1"
        >Default (dropdown)</v-btn
      >
    ...
    <div class="mb-1 mt-4 d-flex align-center gap-1">
      <span class="text-subtitle-2 text-grey-darken-1 mr-2 font-weight-bold"
        >Cascade filter</span
      >
    ...
    >
      ...
      <v-btn
        ...
        :title="
          availableParentSelectFields.length === 0
...
const isCustomAppearance = ref(
  !!(
    props.local.appearance &&
    props.local.appearance.replace("minimal", "").trim()
  ),
);
...
const appearanceMode = computed(() =>
  isCustomAppearance.value ? "custom" : "default",
);
...
const appearanceValue = computed({
  get() {
    ...
    return props.local.appearance
      .split(" ")
      .filter((v) => v && v !== "minimal");
  },
  set(val) {
